...
class PopupsWatchdog(BaseWatchdog):
	...
	async def on_TabCreatedEvent(self, event: TabCreatedEvent) -> None:
		...
		try:
			...
			self.logger.info(f"✅ Successfully registered Page.javascriptDialogOpening handler for session {cdp_session.session_id}")
	...
	async def on_DialogOpenedEvent(self, event: DialogOpenedEvent) -> None:
		...
		try:
			...
			await asyncio.wait_for(self.browser_session._cdp_client_root.send.Page.handleJavaScriptDialog(
				params={'accept': True},
				session_id=cdp_session.session_id,
			), timeout=5.0)
			# CRITICAL: you must either wait for Page.javascriptDialogClosed or wait for some fixed time here
			await asyncio.sleep(0.2)
			assert (await cdp_session.cdp_client.send.Runtime.evaluate(
