...
async def test_watchdog_network_timeout(httpserver):
	...
	try:
		...
		assert timeout_error is not None, f"NetworkTimeout event was not emitted within 3 seconds. Got {len(error_events)} error events"
		...
		if session._browser:
			...
			try:
				...
				disconnect_error = await session.event_bus.expect(
					BrowserErrorEvent,
					predicate=lambda e: e.error_type == 'BrowserDisconnected',
					timeout=2.0
				)
