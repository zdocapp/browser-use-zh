...
- **🔐 Preserve Authentication**: Keep your existing login sessions
- **🧩 Use Extensions**: Access your installed browser extensions  
- **🐛 Debug Easily**: See exactly what the agent is doing in real-time
- **⚡ Faster Startup**: Skip browser launch time
- **🔒 Enterprise Settings**: Use browsers with corporate policies
...
session = Browser(browser_profile=profile)
```
...
test_session = Browser(browser_profile=profiles["testing"])
```
...
# Login manually, then use the profile path in Browser
```
...
agent1 = Agent(task="Check email", browser=session)
await agent1.run()

agent2 = Agent(task="Update profile", browser=session) 
await agent2.run()
