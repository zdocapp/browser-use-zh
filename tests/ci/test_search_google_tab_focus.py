...
async def test_search_google_creates_and_focuses_new_tab(browser_session: BrowserSession):
	...
	initial_state_event = browser_session.event_bus.dispatch(
...
async def test_multiple_new_tabs_focus_on_latest(browser_session: BrowserSession):
	...
	nav1_event = browser_session.event_bus.dispatch(
		NavigateToUrlEvent(url='https://example.com', new_tab=True)
	)
	await nav1_event
	...
	nav2_event = browser_session.event_bus.dispatch(
		NavigateToUrlEvent(url='https://github.com', new_tab=True)
	)
	await nav2_event
	await asyncio.sleep(0.5)
