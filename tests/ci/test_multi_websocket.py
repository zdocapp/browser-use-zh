...
async def test_dedicated_websocket_per_target(test_html):
	...
	try:
		...
		session2 = await browser_session.get_or_create_cdp_session(target_id, focus=False, new_socket=True)
		...
		result1 = await session1.cdp_client.send.Runtime.evaluate(
			params={'expression': 'document.title'}, session_id=session1.session_id
		)
		assert result1['result']['value'] == 'Main Page'

		# Test session2
		result2 = await session2.cdp_client.send.Runtime.evaluate(
			params={'expression': 'document.title'}, session_id=session2.session_id
		)
...
async def test_websocket_cleanup(test_html):
	...
	try:
		...
		for i in range(3):
			...
			session = await browser_session.get_or_create_cdp_session(target_id, focus=False, new_socket=True)
...
async def test_mixed_websocket_sessions(test_html):
	...
	try:
		...
		session_shared1 = await browser_session.get_or_create_cdp_session(target_ids[0], focus=False, new_socket=False)
		session_dedicated1 = await browser_session.get_or_create_cdp_session(target_ids[1], focus=False, new_socket=True)
		session_shared2 = await browser_session.get_or_create_cdp_session(target_ids[2], focus=False, new_socket=False)
		session_dedicated2 = await browser_session.get_or_create_cdp_session(target_ids[3], focus=False, new_socket=True)
