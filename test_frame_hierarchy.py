    ...
    try:
        ...
        if iframe_targets:
            ...
            for iframe_target in iframe_targets:
                ...
                try:
                    ...
                    print(f"  ⚠️  This frame runs in a separate process (OOPIF)")
        ...
        for i, (frame_id, frame_info) in enumerate(all_frames.items()):
            ...
            if not parent and 'v0-simple-landing' in url:
                print(f"      ❌ PROBLEM: Cross-origin frame incorrectly marked as root!")
                ...
                if any(url in t.get('url', '') for t in page_targets):
                    print(f"      ✅ Correctly identified as root frame")
            
            if is_cross:
                print(f"      🔸 This is a cross-origin frame (OOPIF)")
