...
async def test_permissions_are_granted_on_connect(httpserver, caplog):
	...
	grant_logs = [record for record in caplog.records if "Granting browser permissions" in record.message or "Browser.grantPermissions" in record.message]
	assert len(grant_logs) > 0, "Expected Browser.grantPermissions to be called"
	
	# Check that the correct permission was granted
	geo_permission_logs = [record for record in caplog.records if "geolocation" in record.message and ("granting" in record.message.lower() or "granted" in record.message.lower())]
...
async def test_multiple_permissions_are_granted(caplog):
	...
	profile = BrowserProfile(
		permissions=['geolocation', 'clipboardReadWrite', 'notifications'],
		headless=True
	)
	...
	# Check that permissions were granted
	grant_logs = [record for record in caplog.records if "Granting browser permissions" in record.message or "Successfully granted permissions" in record.message]
...
async def test_permissions_watchdog_handles_invalid_permissions(caplog):
	...
	profile = BrowserProfile(
		permissions=['geolocation', 'invalid-permission', 'clipboardReadWrite'],
		headless=True
	)
	...
	warning_logs = [record for record in caplog.records if "invalid-permission" in record.message or "Unknown permission" in record.message]
