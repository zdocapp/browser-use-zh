...
You should also configure [`Browser(allowed_domains=...)`](https://docs.browser-use.com/customize/browser-settings#allowed-domains) to prevent the Agent from visiting URLs not needed for the task.
...
agent = Agent(
    ...
    browser=browser, # Pass the restricted browser to limit URLs Agent can visit
...
- Always restrict [`Browser(allowed_domains=[...])`](https://docs.browser-use.com/customize/browser-settings#allowed-domains) to only the domains the agent needs to visit to accomplish its task. This helps guard against prompt injection attacks, jailbreaks, and LLM mistakes.
...
```python
agent = Agent(..., browser=Browser(storage_state='./auth.json'))
```
...
For convenience the system will validate that all domain patterns used in `Agent(sensitive_data)` are also included in `Browser(allowed_domains)`.
