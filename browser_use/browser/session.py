...
class BrowserSession(BaseModel):
	...
	async def _click_element_node(self, element: Any, expect_download: bool = False, new_tab: bool = False) -> str | None:
		...
		if expect_download and self.browser_profile.downloads_path:
			# When expecting a download with CDP connection, let the downloads watchdog handle it
			# and just wait for FileDownloadedEvent
			logger.info('[Session] Expecting download - letting downloads watchdog handle via CDP...')

			# Click the element and wait for download to be handled by watchdog
			if new_tab:
				import sys
			...
			# Wait for FileDownloadedEvent from downloads watchdog
			try:
				download_event = await self.event_bus.expect(FileDownloadedEvent, timeout=15.0)
				logger.info(f'[Session] Download completed via watchdog: {download_event.path}')
				return download_event.path
			except TimeoutError:
				logger.warning('[Session] Expected download but no FileDownloadedEvent received within timeout')
				return None
		else:
