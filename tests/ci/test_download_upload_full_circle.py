...
from browser_use.browser.events import FileDownloadedEvent, BrowserStateRequestEvent
from browser_use.browser.profile import BrowserProfile
from browser_use.controller.service import Controller
from browser_use.controller.views import GoToUrlAction, ClickElementAction, UploadFileAction
...
def download_upload_server():
	...
	server.expect_request('/upload-page').respond_with_data(
		upload_page_html,
	...
	server.expect_request('/download-page').respond_with_data(
		download_page_html,
		content_type='text/html'
	)
	...
	async def test_download_then_upload_with_hash_verification(self, download_upload_server):
		...
		with tempfile.TemporaryDirectory() as tmpdir:
			...
			try:
				...
				result = await controller.act(
					ClickActionModel(click_element_by_index=ClickElementAction(index=download_link_index)),
					browser_session
				)
				...
				result = await controller.act(
					GoToUrlActionModel(go_to_url=GoToUrlAction(url=f'{base_url}/upload-page', new_tab=True)),
					browser_session
				)
				...
				result = await controller.act(
					UploadActionModel(upload_file_to_element=UploadFileAction(
				...
				for idx, element in state_result.dom_state.selector_map.items():
					if element.tag_name and element.tag_name.lower() == 'button' and element.attributes and element.attributes.get('id') == 'submitButton':
				...
				result = await controller.act(
					ClickActionModel(click_element_by_index=ClickElementAction(index=submit_button_index)),
					browser_session
				)
				...
				assert upload_info.get('fileName') == 'test-file.txt', f"Uploaded filename mismatch: {upload_info.get('fileName')}"
				...
				if download_upload_server.uploaded_files:
					...
					print(f"✅ Server-side verification passed!")
