...
```python
def simple_tool() -> str:
    return "Hello, world!"  # Keep in context for all future steps (recommended)
```
This is equivalent to: `ActionResult(extracted_content="Hello, world!")` and `ActionResult(long_term_memory="Hello, world!")` without any other properties.

### 2. Long content - show once, remember subset
```python
return ActionResult(
...
### 3. Long content - show never, remember subset
```python
return ActionResult(
...
```python
@controller.action(description='Complete the task')
